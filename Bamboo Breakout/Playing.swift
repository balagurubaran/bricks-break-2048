    ...
    if previousState is WaitingForTap {
        if(scene.childNode(withName: BallCategoryName)?.isHidden == false){
            if let ball = scene.childNode(withName: BallCategoryName) as? SKSpriteNode {
                ball.physicsBody?.applyImpulse(CGVector(dx: randomDirection(min: 0, max: 6), dy: abs(randomDirection(min: 10, max: 15))))
            }
        }
  ...
  override func update(deltaTime seconds: TimeInterval) {
    ...
    func ballForce(ball:SKSpriteNode){
        ...
        if let usedata = ball.userData {
            ...
            if ySpeed <= 10.0 {
                physicsBody.applyImpulse(CGVector(dx:physicsBody.velocity.dx, dy: randomDirection(min: 7, max: 15)))
            }
        }
        
//        if (speed < maxSpeed){
//            physicsBody.velocity = CGVector.init(dx: physicsBody.velocity.dx * 1.2, dy: physicsBody.velocity.dy * 1.2)
//        }
